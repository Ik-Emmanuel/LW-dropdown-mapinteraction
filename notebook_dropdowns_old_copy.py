...
def three_step_conversion_for_area_buffer(area_gdf_to_convert):
    """ For some reason, before map click areas can have 
    buffers applied to them, reprojection happend why? """
    ...
    gpd_df_first_conversion  = area_gdf_to_convert.to_crs(epsg=3857)
...
def create_and_display_buffer_include_selection(area_gdf, buffer_distance):
    ...
    if global_area_selection_type and global_area_selection_type == "Select":
        area_gdf = polygon_selected()
        area_gdf = three_step_conversion_for_area_buffer(area_gdf)
